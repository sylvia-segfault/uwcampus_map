...
/**
 * This represents an immutable path between two distinct positions, particularly
...
public class Path<T> implements Iterable<Path<T>.Segment> {
    ...
    // AF(this) =
    //      The beginning of the path => start
    //      each "step" along the path between each position => elements of list path, where
    //          path.get(0) is the first step from the start to an intermediate position, and
    //          path.get(path.size() - 1) is the final step from an intermediate position to the end
    //      total cost along the path => cost
    //      the destination position in this path, opposite the start position => getEnd()
    ...
    /**
     * The ordered sequence of segments representing a path between different positions.
     */
    private List<Segment> path;

    /**
     * Creates a new, empty path containing its start position. Essentially this represents a path
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last position
     ...
     * segmentCost}.
     *
     * @param newEnd      The position being added at the end of the segment being appended to this path
    ...
    /**
     * @return The position at the beginning of this path.
     */
    public T getStart() {
        return start;
    }
    ...
    /**
     * @return The position at the end of this path, which may be the start position if this path
     * contains no segments (i.e. this path is from the start position to itself).
     */
    public T getEnd() {
        if (path.size() == 0) {
            return start;
     ...
     * position and ending at the end position . In the case that this path represents a path between
     * the start position and itself, this iterator contains no elements. This iterator does not
     ...
     * both paths are empty, they are only equal if their starting position is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between different positions.
     * Segments are immutable parts of a larger path that cannot be instantiated directly, and
     * are created as part of larger paths by calling Path#extend(<T></T>, double).
     */
    public class Segment {
         ...
         * @param end   The ending position of this segment.
         * @param cost  The cost of travelling this segment.
         * @throws NullPointerException     if either position is null.
         * @throws IllegalArgumentException if cost is infinite or NaN
         */
        private Segment(T start, T end, double cost) {
            if (start == null || end == null) {
                throw new NullPointerException("Segments cannot have null positions.");
            }
        ...
        /**
         * @return The beginning position of this segment.
         */
        public T getStart() {
            // Note: We assume the object <T> is immutable, this isn't rep exposure.
            return this.start;
        }
